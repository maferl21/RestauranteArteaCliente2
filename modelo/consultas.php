    ...
    class Consultas{
        ...
        function insertarPedido($idOrden, $idProd, $cant, $total){
            ...
            if ($idOrden == "" OR $idProd == "" OR $cant == 0 OR $total == 0)
                ...
                if ($obConexion->conectar()){
                    $sQuery = "INSERT INTO detalle_orden (id_orden,id_prod,cantidad,total) 
                        VALUES ('".$idOrden."', '".$idProd."', '".$cant."','".$total."')";
        ...
        function modificarOrden($idOrden, $cant, $totalOrden){//, $idForma, $idRecep){
            $obConexion=new Conexion();
            $sQuery="";
            $nAfectados=-1;
            if ($idOrden == "")
